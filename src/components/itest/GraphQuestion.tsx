...
export function GraphQuestionCreator({
  ...
  function getDefaultGraphData(type: string) {
    if (type === 'pie') {
      ...
      return {
        a: 1, // slope: y = ax + b
        b: 0, // intercept
        isDirect: false // direct proportionality (y = kx)
      };
    } else { // inverse
      return {
        k: 6 // constant: y = k/x
  ...
  const handleTypeChange = (newType: 'pie' | 'bar' | 'linear' | 'inverse') => {
    ...
    else if (newType === 'inverse') defaultAns = { k: 6 };
  ...
  const updateLinearParams = (a: number, b: number, isDirect: boolean = false) => {
    const finalB = isDirect ? 0 : b;
    onChange({
      graphData: { ...graphData, a, b: finalB, isDirect },
      correctAnswer: { a, b: finalB }
    });
  };

  const updateInverseParam = (k: number) => {
    onChange({
      graphData: { ...graphData, k },
      correctAnswer: { k }
    });
  };
  ...
  return (
    <div className="space-y-6 mt-4 p-5 bg-white border border-gray-100 rounded-2xl shadow-sm">
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {[
            ...
            { id: 'inverse', label: 'Nepřímá úměrnost' },
          ].map((t) => (
            <button
              key={t.id}
              type="button"
      ...
      {graphType === 'linear' && (
        <div className="space-y-4">
          <h4 className="font-bold text-gray-700 border-b pb-2">Lineární funkce / Přímá úměrnost</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id="isDirectCheck"
                  checked={graphData.isDirect || false}
                  onChange={(e) => updateLinearParams(graphData.a, graphData.b, e.target.checked)}
                  className="rounded border-gray-300 text-primary h-4 w-4"
                />
                <label htmlFor="isDirectCheck" className="text-sm font-bold text-gray-700 cursor-pointer">
                  Přímá úměrnost (prochází počátkem [0,0])
                </label>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    step="0.5"
                    value={graphData.a}
                    onChange={(e) => updateLinearParams(parseFloat(e.target.value) || 0, graphData.b, graphData.isDirect)}
                    className="font-bold"
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    step="1"
                    disabled={graphData.isDirect}
                    value={graphData.isDirect ? 0 : graphData.b}
                    onChange={(e) => updateLinearParams(graphData.a, parseFloat(e.target.value) || 0, graphData.isDirect)}
                    className="font-bold"
                  />
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border flex flex-col justify-center text-center">
                ...
                <span className="text-2xl font-black text-primary mt-1">
                  ...
                  {graphData.isDirect ? '' : (graphData.b > 0 ? ` + ${graphData.b}` : (graphData.b < 0 ? ` - ${Math.abs(graphData.b)}` : ''))}
                </span>
              </div>
            </div>
            ...
            <div className="flex justify-center">
              <svg width="240" height="240" viewBox="0 0 360 360" className="border rounded-2xl bg-white shadow-inner">
                <CoordinateGridLines />
                <CoordinateLinePlot a={graphData.a} b={graphData.isDirect ? 0 : graphData.b} color="#3B82F6" isDashed={false} />
              </svg>
            </div>
          </div>
        </div>
      ...
      {/* INDIRECT PROPORTIONALITY CONFIGURATION */}
      {graphType === 'inverse' && (
        <div className="space-y-4">
          <h4 className="font-bold text-gray-700 border-b pb-2">Nepřímá úměrnost (y = k / x)</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-500 uppercase">Koeficient k (konstanta)</label>
                <Input
                  type="number"
                  min="-20"
                  max="20"
                  step="1"
                  value={graphData.k || 6}
                  onChange={(e) => updateInverseParam(parseInt(e.target.value) || 1)}
                  className="font-bold"
                />
              </div>
            ...
            <div className="flex justify-center">
              <svg width="240" height="240" viewBox="0 0 360 360" className="border rounded-2xl bg-white shadow-inner">
                <CoordinateGridLines />
                <CoordinateHyperbolaPlot k={graphData.k || 6} color="#3B82F6" isDashed={false} />
              </svg>
            </div>
          </div>
        </div>
...
export function GraphQuestionStudent({
  ...
  useEffect(() => {
    if (value === undefined || value === null) {
      ...
      } else if (graphType === 'linear' || graphType === 'inverse') {
  ...
  // Render Coordinate Grid Graph Student View (Linear / Inverse)
  if (graphType === 'linear' || graphType === 'inverse') {
    ...
    // Calculate student inverse k if points exist
    let studentK: number | null = null;
    if (graphType === 'inverse' && studentPoints.length > 0) {
      const products = studentPoints.map(([x, y]) => x * y);
      const avg = products.reduce((sum, p) => sum + p, 0) / products.length;
      studentK = Math.round(avg);
    }
      ...
      <div className="flex flex-col md:flex-row items-center gap-6 p-4 bg-white rounded-2xl border justify-center">
        ...
        <div className="relative">
          ...
          >
            ...
            {graphType === 'linear' && studentLine && (
              <CoordinateLinePlot a={studentLine.a} b={studentLine.b} color="#EF4444" isDashed={false} />
            )}

            {/* Dynamic hyperbola fitting student points */}
            {graphType === 'inverse' && studentK && studentK !== 0 && (
              <CoordinateHyperbolaPlot k={studentK} color="#EF4444" isDashed={false} />
            )}
          </svg>
        ...
        <div className="flex-1 space-y-4 max-w-sm">
            ...
            <ul className="text-xs text-muted-foreground list-disc list-inside space-y-1">
              ...
              {graphType === 'linear' ? (
                <>
                  ...
                  {graphData.isDirect && <li>Jelikož jde o <strong>přímou úměrnost</strong>, nezapomeňte na bod [0,0]!</li>}
                </>
              ) : (
                <li>Umístěním bodů (doporučeno aspoň 3 body) vytyčíte hyperbolu nepřímé úměrnosti.</li>
              )}
            </ul>
          </div>
          ...
          <div className="bg-slate-50 p-4 rounded-xl border space-y-2">
            ...
            {graphType === 'inverse' && studentK && studentK !== 0 && (
              <div className="pt-2 border-t text-center">
                <span className="text-[10px] font-bold text-slate-400 uppercase block">Vámi vytyčená hyperbola</span>
                <span className="text-sm font-black text-red-600">
                  y = {studentK} / x
                </span>
              </div>
...
export function GraphQuestionEvaluation({
  ...
  // Render Coordinate Grid Graph Evaluation View (Linear / Inverse)
  if (graphType === 'linear' || graphType === 'inverse') {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-slate-50/50 rounded-2xl border justify-center">
        ...
        <div className="flex flex-col items-center bg-white p-4 rounded-xl border space-y-2">
          ...
          <svg width="220" height="220" viewBox={`0 0 ${GRID_SIZE} ${GRID_SIZE}`} className="border rounded-xl bg-white shadow-sm">
            <CoordinateGridLines />
            {graphType === 'linear' && (
              <CoordinateLinePlot a={graphData.a || 1} b={graphData.isDirect ? 0 : (graphData.b || 0)} color="#10B981" isDashed={false} />
            )}
            {graphType === 'inverse' && (
              <CoordinateHyperbolaPlot k={graphData.k || 6} color="#10B981" isDashed={false} />
            )}
          </svg>
          <div className="text-center pt-1 text-xs font-black text-green-700">
            {graphType === 'linear' ? (
              <span>y = {graphData.a !== 1 ? (graphData.a === -1 ? '-' : graphData.a) : ''}x {graphData.isDirect ? '' : (graphData.b > 0 ? `+ ${graphData.b}` : (graphData.b < 0 ? `- ${Math.abs(graphData.b)}` : ''))}</span>
            ) : (
              <span>y = {graphData.k || 6} / x</span>
            )}
          </div>
        </div>
      </div>
